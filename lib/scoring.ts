...
export function score(
  ...
  const activeFVG = detection.fvgs
    .filter(f => !f.filled)
    .find(f => lastClose >= Math.min(f.bottom, f.top) && lastClose <= Math.max(f.bottom, f.top));

  const paSignal: SignalBadge = {
    label: `PA ${overallBias === 'bullish' ? '▲' : overallBias === 'bearish' ? '▼' : '◆'}`,
    type: 'bos',
    detail: `BOS ${detection.breakOfStructures.length}, Sweeps ${detection.liquiditySweeps.length}, FVGs ${detection.fvgs.filter(f => !f.filled).length}`,
  };

  const fvgStateSignal: SignalBadge = {
    label: activeFVG ? `Inside ${activeFVG.direction === 'bullish' ? 'Bull' : 'Bear'} FVG` : 'Out of FVG',
    type: 'fvg',
    detail: activeFVG
      ? `${Math.min(activeFVG.bottom, activeFVG.top).toFixed(2)} - ${Math.max(activeFVG.bottom, activeFVG.top).toFixed(2)}`
      : 'No open FVG at current price',
  };
